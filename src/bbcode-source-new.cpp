...
// ---------------------------------------------------------------------------
// Render — per-segment/per-char GPU drawing with all 11 effects
//
// RAINBOW FIX: For rainbow segments, we compute the HSV color per-char
// and apply it as a GPU color multiply. The textures are pre-rendered
// in white, so multiplying by the rainbow color produces the correct
// result with zero GDI overhead per frame.
// ---------------------------------------------------------------------------
static void bbcode_text_render(void *data, gs_effect_t *effect)
{
    ...
    struct vec4 white_color;
    vec4_set(&white_color, 1.0f, 1.0f, 1.0f, 1.0f);
    ...
    for (auto &seg : s->segments) {
        ...
        if (!per_char) {
            ...
            // Reset color to white (no tint) for non-rainbow segments
            if (color_param)
                gs_effect_set_vec4(color_param, &white_color);
            ...
            if (seg.fx & FX_SCROLL) {
                ...
                gs_effect_set_texture(image, seg.tex);
            ...
            for (int ci = 0; ci < (int)seg.chars.size(); ci++) {
                ...
                // RAINBOW FIX: Apply color tint via GPU instead of re-rendering
                if (seg.fx & FX_RAINBOW) {
                    ...
                    if (color_param)
                        gs_effect_set_vec4(color_param, &rainbow_col);
                } else {
                    // No rainbow — reset to white (no tint)
                    if (color_param)
                        gs_effect_set_vec4(color_param, &white_color);
                }
                ...
                if (use_curve) {
                    ...
                    gs_effect_set_texture(image, c.tex);
            ...
            // Reset color after animated segment (in case rainbow was active)
            if (color_param)
                gs_effect_set_vec4(color_param, &white_color);
        }
    }
